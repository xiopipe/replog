---
description: Propose and apply a schema change keeping docs/sql and the spec in sync.
...
1. Review @docs/Architecture.md and @docs/sql/ to locate the change.
2. Propose the SQL migration: idempotent, RLS intact, uuid / `updated_at` / soft delete conventions.
3. Update `docs/sql/` **and** `docs/Architecture.md` (the spec must stay the truth).
