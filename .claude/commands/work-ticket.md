...
3. In the worktree: `cp ../../fitrack/.env .env` then `CI=true pnpm install --frozen-lockfile` (real install — do NOT symlink node_modules; frozen lock keeps jest-mock deterministic; the install runs prepare→husky so hooks fire there).
